...
import static com.deepglint.library.tool.FusConstTool.BYTE;
import static com.deepglint.library.tool.FusConstTool.GB;
import static com.deepglint.library.tool.FusConstTool.KB;
import static com.deepglint.library.tool.FusConstTool.MB;

/**
 * Created by gaofengdeng 2020/4/18
 **/
public class FusDataTool {
     ...
     * @param byteNum 字节数
     * @param unit    <ul>
     *                <li>{@link FusConstTool.MemoryUnit#BYTE}: 字节</li>
     *                <li>{@link FusConstTool.MemoryUnit#KB}  : 千字节</li>
     *                <li>{@link FusConstTool.MemoryUnit#MB}  : 兆</li>
     *                <li>{@link FusConstTool.MemoryUnit#GB}  : GB</li>
     *                </ul>
     * @return 以unit为单位的size
     */
    public static double byte2Size(long byteNum, FusConstTool.MemoryUnit unit) {
        if (byteNum < 0) {
            return -1;
        }
        switch (unit) {
     ...
     * @return 字节数
     */
    public static long size2Byte(long size, FusConstTool.MemoryUnit unit) {
        if (size < 0) {
            return -1;
        }
        switch (unit) {
    ...
    public static String changeDistance(double length, boolean displayMeter) {
        if (length < 1000) {
            return FusConstants.FORMAT_TWO.format(length) + (displayMeter ? "米" : "");
        } else {
            return FusConstants.FORMAT_TWO.format(length / 1000) + (displayMeter ? "千米" : "");
        }
    }
